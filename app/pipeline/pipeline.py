...
def dataCleaningPipeline(df):
    df.drop(["source" ,"dataset", "nb_annotators", "post_author_country_location"], inplace=True, axis=1)
...
df = dataPipeline(df)
df.to_csv("./en_hf_112024_cleaned.csv", index=False)
df.to_pickle("./en_hf_112024_cleaned.pkl")
