...
def compute_token_saliency(model, input_indices, vocab, device="cpu"):
    ...
    for s in saliency:
        s["saliency"] = (s["saliency"] - min_sal) / (max_sal - min_sal) if max_sal > min_sal else 0.0
...
def explain(text):
    # Preprocess
    input_tensor, tokens = preprocess_input(text, word_to_index, max_seq_length=100)  # (1, seq_len)
    attributions = integrated_gradients(model, input_tensor, word_to_index, device=device)
